...
use m_smoother_laplacian_sparse
    ...
    type(t_correlate) :: u_star_u
    real,dimension(:,:,:),allocatable :: RTMimage, RTMimage2, illum, CIGs
    ...
    call alloc(correlate_image,m%nz,m%nx,m%ny,ppg%nimag)
    call alloc(correlate_energy,m%nz,m%nx,m%ny,ppg%nengy)
    call alloc(illum,m%nz,m%nx,m%ny)
    ...
    do i=1,shls%nshots_per_processor
        ...
        call correlate_assemble(u_star_u%ipp,illum)
    ...
    !collect global correlations
    call mpi_allreduce(mpi_in_place,correlate_image,  m%n, mpi_real, mpi_sum, mpiworld%communicator, mpiworld%ierr)
    call mpi_allreduce(mpi_in_place,illum,      m%n, mpi_real, mpi_sum, mpiworld%communicator, mpiworld%ierr)
    call mpi_allreduce(mpi_in_place,CIGs,       m%n*nCIG, mpi_real, mpi_sum, mpiworld%communicator, mpiworld%ierr)

    !postprocess correlation image
    call laplacian_filter(correlate_image)
    where (correlate_image(:,:,:,1) > 0)
        correlate_image(:,:,:,1) = correlate_image(:,:,:,1) / illum(:,:,:)
    endwhere
