...
use m_smoother_laplacian_sparse
    ...
    do i=1,shls%nshots_per_processor
        ...
        call adjsrc_L2sq(shot%dadj) !an minus sign is given inside
        !however in ppg's imaging subroutines no minus sign is given
        !so add this minus sign here
        shot%dadj=-shot%dadj
...
if(.false.) then
    ...
    ! pause
        ...
        if(index(corrs,'DR')>0) then
            ...
            call ppg%init_field(fld_u,name='fld_u');    call fld_u%ignite
            call ppg%init_field(fld_du,name='fld_du')
            call ppg%forward_scattering(fld_du,fld_u,Imag_as_adjsrc); call fld_du%acquire !shot%dsyn = R δu
            !adjoint modeling with adjsource = R δu
            !Aᴴ a = RᴴR δu
            !grad = u★a
