...
use m_cpml
    ...
    type,public :: t_propagator
        ...
        character(i_str_xxlen) :: info = &
            ...
            'based on Kolsky-Futtermann model'//s_NL// &
            ...
            '   -> dt ≤ 0.5*Vmax/dx'//s_NL// &
            'Required model attributes: vp, qp, rho'//s_NL// &
        ...
        real,dimension(:,:,:),allocatable :: koppa2,koppa1,koppa0
        real,dimension(:,:,:),allocatable :: coppa2,coppa1,coppa0
    ...
    character(:),allocatable :: s_KFmodel_behavior
    ...
    subroutine check_model(self)
        ...
        if(index(self%info,'qp')>0  .and. .not. allocated(m%qp)) then
            call alloc(m%qp,m%nz,m%nx,m%ny,o_init=2000.)
            call warn('Constant qp model (2000) is allocated by propagator.')
        endif
    ...
    subroutine init(self)
        class(t_propagator) :: self
        real :: a, b, d, e
        real,dimension(:,:,:),allocatable :: piQ,C2,C1,C0
        ...
        !Kolsky-Futtermann model
        s_KFmodel_behavior=setup%get_str('KFMODEL_BEHAVIOR',o_default='dispersion dissipation')
        if(index(s_KFmodel_behavior,'dispersion')>0)
            a=5.7356; b=-762.1606; d=4.6054e4
        else
            a=0.; b=0.; d=0.
            call hud('KF model induces NO dispersion')
        end
        if(index(s_KFmodel_behavior,'dissipation')>0)
            e=1
        else
            e=0
            call hud('KF model induces NO dissipation')
        end

        piQ=r_pi*self%qp
        C2=1-2*a/piQ -c_i*e/self%qp
        C1=  2*b/piQ
        C0=  2*d/piQ

        self%koppa2=1/(      C2-c_i*dt/2*C1)
        self%coppa2=1/(conj(C2)-c_i*dt/2*C1)           
        
        self%koppa0=-(dt2*C0-2*     C2 ) *self%koppa2
        self%coppa0=-(dt2*C0-2*conj(C2)) *self%coppa2
        
        self%koppa1=-(     C2 +c_i*dt/2*C1)*self%koppa2
        self%coppa1=-(conj(C2)+c_i*dt/2*C1)*self%coppa2
        
        deallocate(piQ,C2,C1,C0)
    ...
    !
    !------- Kolsky-Futtermann model -------
    !Wave eqn in the frequency domain:  -ϰω²u -∇·b∇u = f
    !where u=p=tr(s)=szz=sxx=syy is (hydrostatic) pressure,
    !ω is angular frequency, b=ρ⁻¹ is buoyancy,
    !ϰ=κ⁻¹ is the frequency-dependent bulk compliance (inverse of modulus):
    !ϰ(ω) = ϰ(ω₀)[1 -2/πQ*ln(ω/ω₀) +isgn(ω)/Q]
    !                dispersion     dissipation
    !and ω₀ is a reference frequency, ϰ(ω₀) is he relaxed bulk compliance
    !So
    !-ϰ(ω₀)[ω²u -2/πQ*ω²ln(ω/ω₀)u +ω²/Q*isgn(ω)u] -∇·b∇u = f
    !Define v=isgn(ω)u, the Hilbert transform of u, and
    !U=u+iv the analytic function of u, then
    !-ϰ(ω₀)[ω² -2/πQ*ω²ln(ω/ω₀) -i/Q]U -∇·b∇U = f
    !
    !Approximate ω²ln(ω/ω₀) ≈ aω² +bω +d, at ω₀=1Hz (Yang & Zhu, 2021, GJI)
    !where a=5.7356, b=-762.1606, d=4.6054e4
    !and isgn(ω)/Q -> e*isgn(ω)/Q
    !Then
    !-ϰ(ω₀)[(1-2a/πQ-ei/Q)ω² -(2b/πQ)*ω -2d/πQ]U -∇·b∇U = f
    !If a=b=d=0: no dispersion
    !If e=0: no dissipation
    !
    !In the time domain,
    !ϰ(ω₀)[C₂∂ₜ²U -iC₁∂ₜU +C₀U] -∇·b∇U = f
    !where C₂=1-2a/πQ-i*e/Q, C₁=2b/πQ, C₀=2d/πQ
    !
    !PDE:      L U = ϰ₀[    C₂∂ₜ² -iC₁∂ₜ +C₀]U - ∇·b∇U = f
    !Adjoint:  LᵀA = ϰ₀[conjC₂∂ₜ² -iC₁∂ₜ +C₀]A - ∇·b∇A = d
    !where a=pᵃ is the adjoint field
    !
    !------- Modeling -------
    !Time discretization:
    !∂ₜ²U => (Unext -2Ucurr +Uprev)/dt²
    !∂ₜU  => (Unext         -Uprev)/(2dt)
    !C₂(Unext-2Ucurr+Uprev)/dt²  -iC₁(Unext-Uprev)/(2dt) +C₀Ucurr =    κ₀*lap + κ₀*f
    !C₂(Unext-2Ucurr+Uprev) -idt/2*C₁(Unext-Uprev)    +dt²C₀Ucurr = dt²κ₀*lap + κ₀*f*dt²
    !(C₂-idt/2*C₁)Unext +(dt²C₀-2C₂)Ucurr +(C₂+idt/2*C₁)Uprev     = dt²κ₀*lap + κ₀*f*dt²
    !            Unext =          ϙ₀Ucurr +           ϙ₁Uprev     +dt²ϙ₂κ₀*lap + ϙ₂κ₀*f*dt²
    !where ϙ₂=1/(C₂-idt/2*C₁),
    !      ϙ₀=-(dt²C₀-2C₂)*ϙ₂,
    !      ϙ₁=-(C₂+idt/2*C₁)*ϙ₂
    !
    !conjC₂(Anext-2Acurr+Aprev) -idt/2*C₁(Anext-Aprev)    +dt²C₀Acurr = dt²κ₀*lap + κ₀*d*dt²
    !(cC₂-idt/2*C₁)Anext +(dt²C₀-2cC₂)Acurr +(cC₂+idt/2*C₁)Aprev      = dt²κ₀*lap + κ₀*d*dt²
    !              Anext =         cϙ₀Acurr +           cϙ₁Aprev       +dt²ϙ₂κ₀*lap + cϙ₂κ₀*d*dt²
    !where cϙ₂=1/(conjC₂-idt/2*C₁),
    !      cϙ₀=-(dt²C₀-2conjC₂)*cϙ₂,
    !      cϙ₁=-(conjC₂+idt/2*C₁)*cϙ₂
    !
    ...
    !                     |       |       bz      |       |
    !                     |       |       |       |       |
    !                    κ,Q  bx κ,Q  bx κ,Q  bx κ,Q  bx κ,Q
    ...
    !ϰ₀[C2*∂ₜ²U +C1*∂ₜU +C3*U] -∇·b∇U = f

    !ϰ*∂ₜ²p = [∂zᶠ ∂ₓᶠ][bz  ][∂zᵇ]p
    !                  [  bx][∂ₓᵇ]
    !where
    !∂ₜ²*dt² := p^n+1 -2p^n +p^n-1  ~O(t²)
    !∂ₜ *2*dt:= p^n+1       -p^n-1  ~O(t²)
    ...
    subroutine forward(self,fld_v,fld_u)
        class(t_propagator) :: self
        ...
        do it=ift,ilt
            ...
            call cpu_time(tic)
            call self%update_pressure(fld_v,fld_u,time_dir,it)
            call cpu_time(toc)
            tt4=tt4+toc-tic
...
!tobe fixed..
    ...
    subroutine update_pressure(self,v,u,time_dir,it)
        class(t_propagator) :: self
        type(t_field) :: v,u
        ...
        if(time_dir>0.) then !in forward time
            !Unext = ϙ₀Ucurr +ϙ₁Uprev +dt²ϙ₂κ₀*lap
            f%p_next(ifz:ilz,ifx:ilx,:) = C1*(C2*f%p(ifz:ilz,ifx:ilx,:) +C3*f%p_prev(ifz:ilz,ifx:ilx,:) & 
                +dt2*self%kpa(ifz:ilz,ifx:ilx,:)*f%lap(ifz:ilz,ifx:ilx,:) )
        else !in reverse time
            !Unext = ϙ₀Ucurr +ϙ₁Uprev +dt²ϙ₂κ₀*lap
            ! Unext = C1*(C2*Ucurr +C3*Uprev +r2*lap);
