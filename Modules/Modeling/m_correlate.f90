...
use m_computebox
    ...
    type,public :: t_correlate
        ...
        real,dimension(:,:,:),allocatable :: gqp,  giqp
    ...
    subroutine scale(self,scaler)
        ...
        if(allocated(self%giqp))       call scale_copy(self%giqp,scaler)
    ...
    subroutine stack(self)
        ...
        if(allocated(self%giqp))  call mpi_reduce(mpi_in_place, self%giqp,  m%n, mpi_real, mpi_sum, 0, mpiworld%communicator, mpiworld%ierr)
    ...
    subroutine write(self,o_it,o_suffix)
        ...
        if(.not.present(o_it)) then !just write
            ...
            if(allocated(self%giqp))   call sysio_write(self%name//'%giqp'//suf  ,self%giqp,  m%n)
        ...
        if(if_snapshot) then !write snapshots
            ...
            if(o_it==1 .or. mod(o_it,i_snapshot)==0 .or. o_it==nt) then
                ...
                if(allocated(self%giqp))  call sysio_write('snap_'//self%name//'%giqp'//suf, self%giqp, m%n,o_mode='append')
    ...
    subroutine final(self)
        ...
        call dealloc(self%gqp,  self%giqp)
