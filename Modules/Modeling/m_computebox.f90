...
!                                        Computebox: C+D
    ...
    subroutine project(self,ois_background)
        ...
        call hud('Computebox Size = [ifz,ilz] x [ifx,ilx] x [ify,ily] = '//num2str(self%n))
        call hud('  [ifz,ilz],nz: '//num2str(self%ifz)//' '//num2str(self%ilz)//' '//num2str(self%nz))
        call hud('  [ifx,ilx],nx: '//num2str(self%ifx)//' '//num2str(self%ilx)//' '//num2str(self%nx))
        call hud('  [ify,ilx],ny: '//num2str(self%ify)//' '//num2str(self%ily)//' '//num2str(self%ny))
        call hud('Inner area of Computebox:')
        call hud('  ioz,mz: '//num2str(self%ioz)//' '//num2str(self%mz))
        call hud('  iox,mx: '//num2str(self%iox)//' '//num2str(self%mx))
        call hud('  ioy,my: '//num2str(self%ioy)//' '//num2str(self%my))
        ...
        call hud('vp  '//num2str(minval(self%vp))//' '//num2str(maxval(self%vp)))
        if(allocated(self%vs )) call hud('vs  '//num2str(minval(self%vs))//' '//num2str(maxval(self%vs)))
        call hud('rho '//num2str(minval(self%rho))//' '//num2str(maxval(self%rho)))
        call hud('ip  '//num2str(minval(self%vp*self%rho))//' '//num2str(maxval(self%vp*self%rho)))
        if(allocated(self%eps)) call hud('eps '//num2str(minval(self%eps))//' '//num2str(maxval(self%eps)))
        if(allocated(self%del)) call hud('del '//num2str(minval(self%del))//' '//num2str(maxval(self%del)))
