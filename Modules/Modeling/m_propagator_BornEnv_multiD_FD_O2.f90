...
use m_cpml
    ...
    type,public :: t_propagator
        ...
        character(i_str_xxlen) :: info = &
            ...
            'Basic gradients: gvp2 gtDt gtDs'
        ...
        real,dimension(:,:,:),allocatable :: buoz, buox, buoy, kpa, inv_kpa, tilD_dt2
        ...
        procedure :: inject_pressure_adjoint
   ...
   !========= for FDSG O(dx2,dt2) ===================  
    ...
    subroutine init(self)
        ...
        self%tilD_dt2=cb%tilD*self%dt2
    ...
    subroutine forward_scattering(self,fld_du,fld_u)
        class(t_propagator) :: self
        ...
        do it=ift,ilt
            ...
            call cpu_time(tic)
            call self%inject_pressure(fld_u,time_dir,it)
            call self%inject_pressure_scattering(fld_du,fld_u,time_dir,it)
            call cpu_time(toc)
            tt1=tt1+toc-tic
            ...
            ! if(fld_u%if_will_reconstruct) then
                call cpu_time(tic)
                call fld_u%boundary_transport_pressure('save',it)
                call fld_du%boundary_transport_pressure('save',it)
                call cpu_time(toc)
                tt2=tt2+toc-tic
            ! endif
            ...
            call cpu_time(tic)
            call self%update_pressure(fld_u,time_dir,it)
            call self%update_pressure(fld_du,time_dir,it)
            call cpu_time(toc)
            tt4=tt4+toc-tic
            ...
            call cpu_time(tic)
            call self%evolve_pressure(fld_u,time_dir,it)
            call self%evolve_pressure(fld_du,time_dir,it)
            call cpu_time(toc)
            tt5=tt5+toc-tic
    ...
    subroutine adjoint_tilD(self,fld_a2,fld_u,F2_star_E)
        class(t_propagator) :: self
        type(t_field) :: fld_a2,fld_u
        type(t_correlation) :: F2_star_E
        ...
        do it=ilt,ift,int(time_dir)
            ...
            call cpu_time(tic)
            call self%inject_pressure(fld_a2,time_dir,it)
            call cpu_time(toc)
            tt6=tt6+toc-tic

            !adjoint step 5
            call cpu_time(tic)
            call self%evolve_pressure(fld_a2,time_dir,it)
            call cpu_time(toc)
            tt7=tt7+toc-tic

            !adjoint step 4: s^it+1.5 -> s^it+0.5
            call cpu_time(tic)
            call self%update_pressure(fld_a2,time_dir,it)
            call cpu_time(toc)
            tt8=tt8+toc-tic
            ...
            if(mod(it,irdt)==0) then
                call cpu_time(tic)
                call gradient_tilD(fld_a2,fld_u,F2_star_E,it)
                call cpu_time(toc)
                tt11=tt11+toc-tic
            endif
            ...
            call fld_u%write_ext(it,'corr_F2_star_E',F2_star_E%sp_rp,size(F2_star_E%sp_rp))
        ...
        ! cb%corr(1,:,:,:) = cb%corr(2,:,:,:)
        F2_star_E%sp_rp = F2_star_E%sp_rp*m%cell_volume*rdt
        F2_star_E%sp_rp(1,:,:,:) = F2_star_E%sp_rp(2,:,:,:)
    ...
    subroutine adjoint_vp2(self,fld_a1,fld_a2,fld_du,fld_u,F1_star_lapE,F2_star_lapdE)
        class(t_propagator) :: self
        type(t_field) :: fld_a1,fld_a2,fld_du,fld_u
        type(t_correlation) :: F1_star_lapE,F2_star_lapdE
        ...
        do it=ilt,ift,int(time_dir)
            ...
            ! if(present(o_sf)) then
                ...
                call cpu_time(tic)
                call fld_u%boundary_transport_pressure('load',it)
                call fld_du%boundary_transport_pressure('load',it)
                call cpu_time(toc)
                tt2=tt2+toc-tic
            ...
            !adjoint step 6: inject to s^it+1.5 at receivers
            call cpu_time(tic)
            call self%inject_pressure_adjoint(fld_a1,fld_a2,time_dir,it)
            call cpu_time(toc)
            tt6=tt6+toc-tic

            !adjoint step 5
            call cpu_time(tic)
            call self%evolve_pressure(fld_a1,time_dir,it)
            call self%evolve_pressure(fld_a2,time_dir,it)
            call cpu_time(toc)
            tt7=tt7+toc-tic
            ...
            !adjoint step 4: s^it+1.5 -> s^it+0.5
            call cpu_time(tic)
            call self%update_pressure(fld_a1,time_dir,it)
            call self%update_pressure(fld_a2,time_dir,it)
            call cpu_time(toc)
            tt8=tt8+toc-tic
            ...
            if(mod(it,irdt)==0) then
                call cpu_time(tic)
                call gradient_vp2(fld_a1,fld_u, F1_star_lapE ,it)
                call gradient_vp2(fld_a2,fld_du,F2_star_lapdE,it)
                call cpu_time(toc)
                tt11=tt11+toc-tic
            endif
            ...
            call fld_u%write_ext(it,'corr_F1_star_lapE' ,F1_star_lapE %sp_rp,size(F1_star_lapE %sp_rp))
            call fld_u%write_ext(it,'corr_F2_star_lapdE',F2_star_lapdE%sp_rp,size(F2_star_lapdE%sp_rp))
        ...
        ! !preparing for cb%project_back
        ! cb%corr(1,:,:,:) = cb%corr(2,:,:,:)
        F1_star_lapE%sp_rp = F1_star_lapE%sp_rp*m%cell_volume*rdt
        F1_star_lapE%sp_rp(1,:,:,:) = F1_star_lapE%sp_rp(2,:,:,:)
    ...
    subroutine inject_pressure_scattering(self,fld_d,fld,time_dir,it)
        ...
        if(m%is_cubic) then !1/3D
            ...
            fld_d%p(1:cb%mz,1:cb%mx,1:1) = fld_d%p(1:cb%mz,1:cb%mx,1:1) +         self%tilD_dt2*(fld%p(1:cb%mz,1:cb%mx,1:1)-fld%p_prev(1:cb%mz,1:cb%mx,1:1))
        endif


    end subroutine

    subroutine inject_pressure_adjoint(self,fld_a1,fld_a2,time_dir,it)
        class(t_propagator) :: self
        type(t_field) :: fld_a1,fld_a2
        real :: time_dir

call error("inject_pressure_adjoint hasn't been updated!!")
        if(m%is_cubic) then !1/3D
            fld_a1%p(1:cb%mz,1:cb%mx,1:cb%my) = fld_a1%p(1:cb%mz,1:cb%mx,1:cb%my) +         self%tilD_dt2*fld_a1%p(1:cb%mz,1:cb%mx,1:cb%my)
        else !2D
            fld_a1%p(1:cb%mz,1:cb%mx,1:1) = fld_a1%p(1:cb%mz,1:cb%mx,1:1) +         self%tilD_dt2*(fld_a1%p(1:cb%mz,1:cb%mx,1:1)-fld_a1%p_prev(1:cb%mz,1:cb%mx,1:1))
        endif
    ...
    subroutine update_pressure(self,f,time_dir,it)
        ...
        ! if(allocated(tilD)) then
        !     if(time_dir>0.) then !in forward time
        !         f%p_next(1:cb%mz,1:cb%mx,1:cb%my) = f%p_next(1:cb%mz,1:cb%mx,1:cb%my) + tilD*f%p(1:cb%mz,1:cb%mx,1:cb%my)
        !     else !in reverse time
        !         f%p_prev(1:cb%mz,1:cb%mx,1:cb%my) = f%p_prev(1:cb%mz,1:cb%mx,1:cb%my) + tilD*f%p(1:cb%mz,1:cb%mx,1:cb%my)
        !     endif
        ! endif
    ...
    subroutine gradient_vp2(rf,sf,grad,it)
        ...
        if(m%is_cubic) then
            ...
            !inexact greadient
            call grad2d_vp2(rf%p,sf%lapz,sf%lapx,&
                            grad%sp_rp,          &
                            ifz,ilz,ifx,ilx)
        endif
    ...
    subroutine gradient_tilD(rf,sf,grad,it)
        type(t_field), intent(in) :: rf, sf
        type(t_correlation) :: grad
        ...
        if(m%is_cubic) then
            ...
            call grad2d_tilD(rf%p,sf%p,sf%p_next,&
                              grad%sp_rp,        &
                              ifz,ilz,ifx,ilx)
    ...
    subroutine grad2d_tilD(rf_p,sf_p,sf_p_next,&
                           grad,          &
                           ifz,ilz,ifx,ilx)
        real,dimension(*) :: rf_p,sf_p,sf_p_next
        real,dimension(*) :: grad
        ...
        do ix=ifx,ilx
            ...
            do iz=ifz,ilz
                ...
                grad(j)=grad(j) + rf_p(i)*(sf_p_next(i)-sf_p(i))/dt
    ...
    subroutine grad2d_vp2(rf_p,sf_lapz,sf_lapx,&
                          grad,          &
                          ifz,ilz,ifx,ilx)
