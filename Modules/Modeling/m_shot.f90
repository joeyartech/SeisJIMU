...
use m_model
    ...
    subroutine set_var_space(self,is_fdsg)
        ...
        call hud('=================================')
        call hud(self%sindex//' info:')
        call hud('=================================')
        call hud('  nt,dt: '//num2str(self%nt)//' '//num2str(self%dt))
        call hud('---------------------------------')
        call hud('S/R positions after removing m%oz,ox,iy,')
        call hud('  sz,iz: '//num2str(self%src%z)//' '//num2str(self%src%iz))
        call hud('  sx,ix: '//num2str(self%src%x)//' '//num2str(self%src%ix))
        call hud('  sy,iy: '//num2str(self%src%y)//' '//num2str(self%src%iy))
        call hud('  ifz,ilz: '//num2str(self%src%ifz)//' '//num2str(self%src%ilz))
        call hud('  ifx,ilx: '//num2str(self%src%ifx)//' '//num2str(self%src%ilx))
        call hud('  ify,ily: '//num2str(self%src%ify)//' '//num2str(self%src%ily))
        call hud('---------------------------------')
        call hud('  minmax rz,iz: '//num2str(minval(self%rcv(:)%z))//' '//num2str(maxval(self%rcv(:)%z))//' '//num2str(minval(self%rcv(:)%iz))//' '//num2str(maxval(self%rcv(:)%iz)))
        call hud('  minmax rx,ix: '//num2str(minval(self%rcv(:)%x))//' '//num2str(maxval(self%rcv(:)%x))//' '//num2str(minval(self%rcv(:)%ix))//' '//num2str(maxval(self%rcv(:)%ix)))
        call hud('  minmax ry,iy: '//num2str(minval(self%rcv(:)%y))//' '//num2str(maxval(self%rcv(:)%y))//' '//num2str(minval(self%rcv(:)%iy))//' '//num2str(maxval(self%rcv(:)%iy)))
        call hud('  minmax ifz,ilz: '//num2str(minval(self%rcv(:)%ifz))//' '//num2str(maxval(self%rcv(:)%ifz))//' '//num2str(minval(self%rcv(:)%ilz))//' '//num2str(maxval(self%rcv(:)%ilz)))
        call hud('  minmax ifx,ilx: '//num2str(minval(self%rcv(:)%ifx))//' '//num2str(maxval(self%rcv(:)%ifx))//' '//num2str(minval(self%rcv(:)%ilx))//' '//num2str(maxval(self%rcv(:)%ilx)))
        call hud('  minmax ify,ily: '//num2str(minval(self%rcv(:)%ify))//' '//num2str(maxval(self%rcv(:)%ify))//' '//num2str(minval(self%rcv(:)%ily))//' '//num2str(maxval(self%rcv(:)%ily)))
        call hud('  nrcv: '//num2str(self%nrcv))
        call hud('---------------------------------')
