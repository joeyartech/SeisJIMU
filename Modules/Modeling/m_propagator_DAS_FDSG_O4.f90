...
use, intrinsic :: ieee_arithmetic
    ...
    subroutine init(self,oif_record_adjseismo)
        ...
        if(if_hicks.and.m%is_freesurface) then
            ...
            if(shot%src%iz>1.and.shot%src%ifz<1) then
                call hud(shot%sindex//"'s iz, ifz = "//num2str(shot%src%iz)//', '//num2str(shot%src%ifz))
            ...
            do i=1,shot%nrcv
                ...
                if(shot%rcv(i)%iz>1.and.shot%rcv(i)%ifz<1) then
                    call hud(shot%sindex//"has receiver iz, ifz = "//num2str(shot%src%iz)//', '//num2str(shot%src%ifz))
        ...
        !check mu values
        call hud('ppg%mu sanity: '//num2str(minval(self%mu))//' '//num2str(maxval(self%mu)))
    ...
    subroutine init_field(self,f,name,ois_adjoint,oif_will_reconstruct)
        ...
        if(f%if_will_reconstruct) call f%init_boundary_momenta
...
!            call correlate_assemble(corr%grho, correlate_gradient(:,:,:,1))
    ...
    subroutine forward(self,fld_u)
        ...
        do it=ift,ilt
            if(mod(it,500)==0) then
                ...
                if(mpiworld%is_master) call fld_u%check_value(fld_u%pz)
            endif
        ...
        call hud('Elapsed time to add source momenta  '//num2str(tt1/mpiworld%max_threads))
        call hud('Elapsed time to update momenta      '//num2str(tt2/mpiworld%max_threads))
        call hud('Elapsed time to add source strains  '//num2str(tt3/mpiworld%max_threads))
        call hud('Elapsed time to update strains      '//num2str(tt4/mpiworld%max_threads))
        ...
        call hud('Total elapsed time (min): '//num2str((tt1+tt2+tt3+tt4+tt5+tt6)/60./mpiworld%max_threads))
    ...
    !adjoint_a_star_Du
        ...
        call hud('Elapsed time to update strains           '//num2str(tt2/mpiworld%max_threads))
        call hud('Elapsed time to rm source strains        '//num2str(tt3/mpiworld%max_threads))
        call hud('Elapsed time to update momenta           '//num2str(tt7/mpiworld%max_threads))
        call hud('Elapsed time to rm source momenta        '//num2str(tt8/mpiworld%max_threads))
        call hud('Total elapsed time for forward (min): '//num2str((tt1+tt2+tt3+tt7+tt8)/60./mpiworld%max_threads))
        call hud(' ---------------------------- ')
        call hud('Elapsed time to add adjsource strains    '//num2str(tt4/mpiworld%max_threads))
        call hud('Elapsed time to update adj strains       '//num2str(tt5/mpiworld%max_threads))
        call hud('Elapsed time to add adjsource momenta    '//num2str(tt9/mpiworld%max_threads))
        call hud('Elapsed time to update adj momenta       '//num2str(tt10/mpiworld%max_threads))
        ...
        call hud('Total elapsed time for adjoint&correlate (min): '//num2str((tt4+tt5+tt9+tt10+tt11+tt6)/60./mpiworld%max_threads))
        call hud('Total elapsed time (min): '//num2str((tt1+tt2+tt3+tt7+tt8+tt4+tt5+tt9+tt10+tt11+tt6)/60./mpiworld%max_threads))
        ...
        if(m%is_freesurface) then !FS condition: sz=ss=0.
            ...
            if (FS_method=='stress_image') then !replicates Levandar & Roberttson method under strain system
                ...
                do ix=cb%ifx+1,cb%ilx-2
                    ...
                    factor = -self%lda(1,ix)/self%ldap2mu(1,ix) !Poisson ratio

                    f%ez(1,ix,1) = f%ez(1,ix,1) +time_dir*self%dt * factor*dvx_dx_
