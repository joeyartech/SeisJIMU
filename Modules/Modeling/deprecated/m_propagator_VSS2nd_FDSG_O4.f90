...
use, intrinsic :: ieee_arithmetic
    ...
    type,public :: t_propagator
        ...
        character(i_str_xxlen) :: info = &
            ...
            'Basic gradients: grho(wait) gkpa gmu'
        ...
        real,dimension(:,:),allocatable :: kpa, mu
    ...
    subroutine init_field(self,f,name,ois_adjoint,oif_will_reconstruct)
        ...
        call alloc(f%dz_kpa_V, [cb%ifz,cb%ilz],[cb%ifx,cb%ilx],[cb%ify,cb%ily])
        call alloc(f%dx_kpa_V, [cb%ifz,cb%ilz],[cb%ifx,cb%ilx],[cb%ify,cb%ily])
...
call correlate_assemble(corr%g66, correlate_gradient(:,:,:,66))
    ...
    subroutine update(self,f,time_dir,it)
        ...
        if(m%is_cubic) then
            ...
            call fd2d_laplacian(f%uz,f%ux,&
                                ...
                                f%dz_kpa_V,f%dx_kpa_V,f%dz_mu_Szx,f%dx_mu_Sxz,&
                                f%lapz,f%lapx,&
                                self%kpa,self%mu,&
                                ifz,ilz,ifx,ilx)
    ...
    subroutine final(self)
        ...
        call dealloc(self%buoz, self%buox, self%kpa, self%mu)
    end subroutine
    ...
    subroutine fd2d_laplacian(uz,ux,&
                              ...
                              dz_kpa_V,dx_kpa_V,dz_mu_Szx,dx_mu_Sxz,&
                              lapz,lapx,&
                              kpa,mu,&
                              ifz,ilz,ifx,ilx)
        real,dimension(*) :: uz,ux
        real,dimension(*) :: duz_dz,dux_dx,dux_dz,duz_dx
        real,dimension(*) :: dz_kpa_V,dx_kpa_V,dz_mu_Szx,dx_mu_Sxz
        real,dimension(*) :: kpa,mu,lapz,lapx

        real,dimension(:),allocatable :: kpa_V
        real,dimension(:),allocatable :: mu_Sxz
        real,dimension(:),allocatable :: mu_Szx

        call alloc(kpa_V,  cb%n)
        call alloc(mu_Sxz, cb%n)
        call alloc(mu_Szx, cb%n)
        ...
        do ix = ifx+2,ilx-2
            ...
            do iz = ifz+2,ilz-2
                ...
                duz_dx_ = duz_dx_*cpml%kpa_x_half(ix) + duz_dx(iz_ix)
                 ...
                 kpa_V(iz_ix) = kpa(iz_ix) *(duz_dz_ + dux_dx_)
                mu_Sxz(iz_ix) =  mu(iz_ix) * duz_dx_
                mu_Szx(iz_ix) =  mu(iz_ix) * dux_dz_
        ...
        do ix = ifx+2,ilx-2
            ...
            do iz = ifz+2,ilz-2
                ...
                dz_kpa_V_ = c1z*(kpa_V(iz_ix)-kpa_V(izm1_ix)) +c2z*(kpa_V(izp1_ix)-kpa_V(izm2_ix))  !∂zᵇ
                dx_kpa_V_ = c1x*(kpa_V(iz_ix)-kpa_V(iz_ixm1)) +c2x*(kpa_V(iz_ixp1)-kpa_V(iz_ixm2))  !∂ₓᵇ

                dz_kpa_V(i)=cpml%b_z_half(iz)*dz_kpa_V(i)+cpml%a_z_half(iz)*dz_kpa_V_
                dx_kpa_V(i)=cpml%b_x_half(ix)*dx_kpa_V(i)+cpml%a_x_half(ix)*dx_kpa_V_

                dz_kpa_V_ = dz_kpa_V_*cpml%kpa_z_half(iz) + dz_kpa_V(iz_ix)
                dx_kpa_V_ = dx_kpa_V_*cpml%kpa_x_half(ix) + dx_kpa_V(iz_ix)


                dz_mu_Szx_ = c1z*(mu_Szx(izp1_ix)-mu_Szx(iz_ix)) +c2z*(mu_Szx(izp2_ix)-mu_Szx(izm1_ix))  !∂zᶠ
                dx_mu_Sxz_ = c1x*(mu_Sxz(iz_ixp1)-mu_Sxz(iz_ix)) +c2x*(mu_Sxz(iz_ixp2)-mu_Sxz(iz_ixm1))  !∂ₓᶠ

                dz_mu_Szx(i)=cpml%b_z(iz)*dz_mu_Szx(i)+cpml%a_z(iz)*dz_mu_Szx_
                dx_mu_Sxz(i)=cpml%b_x(ix)*dx_mu_Sxz(i)+cpml%a_x(ix)*dx_mu_Sxz_
                
                dz_mu_Szx_ = dz_mu_Szx_*cpml%kpa_z(iz) + dz_mu_Szx(iz_ix)
                dx_mu_Sxz_ = dx_mu_Sxz_*cpml%kpa_x(ix) + dx_mu_Sxz(iz_ix)
                
                lapz(iz_ix) = dz_kpa_V_ + dx_mu_Sxz_
                lapx(iz_ix) = dx_kpa_V_ + dz_mu_Szx_
