...
!  !! 
    ...
    type t_format_su
        ...
        integer(2) :: ns !assume all traces have same number of samples
        contains
        procedure :: init => init
        final     :: finalize
    end type  
    ...
    subroutine read(self,file,o_sindex)
        ...
        inquire(file=self%file, size=file_size) !file_size in bytes
        file_size=file_size/4 !file_size in sizeof(float), single precision
        
        open(11,file=self%file,action='read',access='stream')
        read(11,pos=115) self%ns  !get number of samples from 1st trace
        if(self%ns<0) self%ns=self%ns+32768*2  !ns keyword is unsigned short in C
        close(11)
        ...
        read(11) (self%hdr(i),self%trs(:,i), itr=1,ntr)
        close(11)
    ...
    subroutine write(self,file,o_sindex)
        ...
        write(12) (self%hdr(i),self%trs(:,i), itr=1,ntr)
        close(12)
