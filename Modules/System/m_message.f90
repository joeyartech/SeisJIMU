...
use m_mpienv
    ...
    public :: shotlog_open, shotlog_close

    !shot log file unit (0 means use stdout)
    integer :: shotlog_unit = 0
    ...
    subroutine hud(msg,o_iproc)
        ...
        if(shotlog_unit > 0) then
            !write to shot log file (all ranks write to their own shot files)
            write(shotlog_unit,*) either(mpiworld%sproc//' : ','',present(o_iproc))//msg
        else
            !write to stdout (only specified rank, default rank 0)
    ...
    subroutine shotlog_open(shot_index, o_dir)
