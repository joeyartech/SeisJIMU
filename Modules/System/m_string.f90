...
module m_string
    ...
    !NOTE: s is fixed-length (not allocatable) to work around ifort 2021
    !do_alloc_assign bug that crashes when assigning arrays of derived types
    !with allocatable character components across module boundaries.
    !Use len_trim(str%s) instead of len(str%s) to get the actual content length.
    type t_string
        character(i_str_xlen) :: s = ''
    end type
    ...
    function strcat(strs,o_glue) result(str)
        ...
        maxlen=len_trim(strs(1)%s)+1
        do k=2,either(n,1,n>1)
            length=len_trim(strs(k)%s)+1
            maxlen=either(maxlen,length,maxlen>=length)
        enddo
        text=repeat(glue,maxlen*n)

        do k=0,n-1
            text(k*maxlen+1:k*maxlen+len_trim(strs(k+1)%s))=trim(strs(k+1)%s)
        enddo
