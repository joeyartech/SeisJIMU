...
use m_empirical
    ...
    type,public :: t_parametrizer
        ...
        character(i_str_xxlen) :: info = &
            'Parameterization: vp-vs-vs0-ip-ip0'
...
!     logical :: is_grho,is_gbuo,is_gkpa,is_gikpa,is_glda,is_gmu
    integer :: i_vp=0, i_vs=0, i_vs0=0, i_ip=0, i_ip0=0
    ...
    subroutine transform(self,o_dir,o_x,o_xprior,o_g)
        ...
        if(present(o_x)) then
            ...
            if(either(o_dir,'m->x',present(o_dir))=='m->x') then
                ...
                if(i_vs0>0) o_x(:,:,:,i_vs0) = (m%vs0      -self%pars(i_vs0)%min)/self%pars(i_vs0)%range
                if(i_ip >0) o_x(:,:,:,i_ip ) = (m%vp*m%rho -self%pars(i_ip )%min)/self%pars(i_ip )%range
                if(i_ip0>0) o_x(:,:,:,i_ip0) = (m%vp*m%rho0-self%pars(i_ip0)%min)/self%pars(i_ip0)%range
...
!                 call empirical_m2x('velocities-impedance')
                ...
                if(i_vs0>0) m%vs0 =  o_x(:,:,:,i_vs0)*self%pars(i_vs0)%range +self%pars(i_vs0)%min
                if(i_ip >0) m%rho = (o_x(:,:,:,i_ip )*self%pars(i_ip )%range +self%pars(i_ip )%min)/m%vp
                if(i_ip0>0) m%rho0= (o_x(:,:,:,i_ip0)*self%pars(i_ip0)%range +self%pars(i_ip0)%min)/m%vp
...
!                 call hud('Parametrizer finds grho glda & gmu')
            ...
            if(index(setup%get_str('JOB',o_mandatory=1),'build Ip')>0) then !high-k
                tmp_gvs =(-2*correlate_gradient(:,:,:,2) + correlate_gradient(:,:,:,3))*2*m%rho*m%vs !rho & vs are smooth
                tmp_gip =(correlate_gradient(:,:,:,2)*m%vp**2 + (-2*correlate_gradient(:,:,:,2)+correlate_gradient(:,:,:,3))*m%vs**2 + correlate_gradient(:,:,:,1))/m%vp !vp, vs are smooth
