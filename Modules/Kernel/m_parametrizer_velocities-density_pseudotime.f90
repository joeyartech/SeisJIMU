    ...
    subroutine transform(self,o_dir,o_x,o_xprior,o_g)
        ...
        if(present(o_xprior)) then
            ...
            do i=1,self%npars
                select case (self%pars(i)%name)
                case ('vp' )
                    call pseudotime_z2t(m%vp_prior,tmp,o_v_z=m%vp)
                    o_x(:,:,:,i) = (tmp -self%pars(i)%min)/self%pars(i)%range
                case ('rho')
                    call pseudotime_z2t(m%rho_prior,tmp,o_v_z=m%vp)
        ...
        if(present(o_g)) then
            ...
            if(.not. is_empirical) then
                do i=1,param%npars
                    ...
                    case ('vp' )
                        call pseudotime_z2t_gradient(m%gradient(:,:,:,2)*2*m%rho*m%vp, &
                            m%vp,v_t,tmp)
                        o_g(:,:,:,i) = tmp
                    case ('rho')
                        call pseudotime_z2t_gradient(m%gradient(:,:,:,2)*m%vp**2 + m%gradient(:,:,:,1), &
                            m%vp,v_t,tmp)
                        o_g(:,:,:,i) = tmp
                    end select
                enddo
            ...
            if(is_gardner) then
                call pseudotime_z2t_gradient((m%gradient(:,:,:,2)*(b+2)/b*m%vp**2 + m%gradient(:,:,:,1))*a*b*m%vp**(b-1), &
                                            m%vp,v_t,tmp)
                o_g(:,:,:,1) = tmp
            endif
