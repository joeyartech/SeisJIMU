...
use m_preconditioner
    ...
    !use m_pseudotime
            ...
            if(smoothings(i)%s=='Laplacian') then
                ...
                do j=1,param%npars
                    ...
                    select case (param%pars(j)%name)
                        ...
                        case default; call smoother_Laplacian_pseudo_nonstationary(qp%g(:,:,:,j),r_c0/sqrt(m%epsr*m%mur))
                    end select
                enddo
            endif
        enddo
        ...
        if(setup%get_bool('IF_PARA_SCAL',o_default='F')) then
        if(param%npars==2)then
            ...
            ! if(param%pars(1)%name=='epsr' .and. param%pars(2)%name=='vs') then
            !     gvp_norm = sum(abs(qp%pg(:,:,:,1)))
            !     gvs_norm = sum(abs(qp%pg(:,:,:,2)))
            !     if(gvp_norm>gvs_norm) qp%pg(:,:,:,2)=qp%pg(:,:,:,2)/gvs_norm*gvp_norm !scale on gvs
            !     if(gvp_norm<gvs_norm) qp%pg(:,:,:,1)=qp%pg(:,:,:,1)/gvp_norm*gvs_norm !scale on gvp
