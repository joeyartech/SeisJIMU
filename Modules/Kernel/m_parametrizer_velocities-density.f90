...
use m_empirical
    ...
    subroutine init(self)
        ...
        loop: do i=1,size(list)
            sublist=split(trim(list(i)%s),o_sep=':') !=[name, min, max]

            select case (trim(sublist(1)%s))
            case ('vp' )
                i_vp=i
                self%pars(i)%name='vp'
            ...
            self%pars(i)%min=str2real(trim(sublist(2)%s))
            self%pars(i)%max=str2real(trim(sublist(3)%s))
