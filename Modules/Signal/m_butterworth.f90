...
! ****************************************************************************!!
    subroutine bfdesign (fpass, apass, fstop, astop, npoles, f3db)
        ! Input parameters
        real, intent(in) :: fpass      ! Pass-band frequency
        real, intent(in) :: apass      ! Pass-band attenuation
        real, intent(in) :: fstop      ! Stop-band frequency
        real, intent(in) :: astop      ! Stop-band attenuation

        ! Output parameters
        integer, intent(out) :: npoles ! Number of poles
        real, intent(out) :: f3db      ! 3 dB frequency

        ! Local variables
        real :: wpass                  ! Warped pass-band frequency
        real :: wstop                  ! Warped stop-band frequency
        real :: fnpoles                ! Float number of poles
        real :: w3db                   ! Warped 3 dB frequency
...
! ****************************************************************************!!
    subroutine bfhighpass (npoles, f3db, n, trace)
        ! Input parameters
        integer, intent(in) :: npoles  ! Number of poles (and zeros)
        real, intent(in) :: f3db       ! 3 dB frequency (Nyquist = 0.5)
        integer, intent(in) :: n       ! Length of array

        ! Input/Output
        real,dimension(n), intent(inout) :: trace  ! Array to be filtered

        ! Local variables
        real,dimension(n) :: tmp       ! Temporary storage for input
        real :: r                      ! Bilinear transform parameter
        real :: a, b1, b2              ! Filter coefficients
        real :: scale                  ! Normalization factor
        real :: theta                  ! Pole angle
        real :: pj, pjm1, pjm2         ! Previous input values
        real :: qjm1, qjm2             ! Previous output values
        integer :: j, jpair            ! Loop counters
