module m_W1norm
!Treating data comparison as an optimal transport problem
!The data distance is measured by the Wasserstein-1 norm
!which is solved by the Kantorovic-Robinstein prime-dual method.
...
use m_proxy
	...
	character(:),allocatable :: s_package
	...
	type,public :: t_KROT
		...
		real :: scale_residual=1.,scal_was
		real :: bound_adjoint
		real :: const
		real :: taper_rfl
	...
	subroutine W1norm_init()

		s_package = setup%get_str("KROT_SDMM_PACKAGE",o_default='fishpack')
		niterate    = setup%get_int("KROT_SDMM_MAX_ITERATE",o_default='50') 
		convergence = setup%get_real("KROT_SDMM_CONVERGENCE",o_default='1e-3')

		scale_seismo = setup%get_real("KROT_SCALE",o_default='1')
		bound_seismo = setup%get_real("KROT_BOUND",o_default='1')

	end subroutine

	subroutine W1norm(fobj_misfit,seismo,n1,n2)

		seismo=seismo*ot%scale_residual

		select case (s_package)
		case ('fishpack')
			call SDMM_fishpack(fobj_misfit,seismo,n1,n2)    !ZW FFT solver for poisson eq, recommended
		end select
	...
	subroutine SDMM_fishpack(fobj_misfit,seismo,n1,n2)
	...
	!   Imported to SeisJIMU
		...
		!allocate(fcost_profile(niterate))
		!Main Loop 
		do i=1,niterate
			...
			call HWSCRT(A,B,Mfish,MBDCND,BDA,BDB,C,D,Nfish,NBDCND,BDC,BDD,&
						ELMBDA,F,IDIMF,PERTRB,ierr)
			call cpu_time(toc)

			if(mype==0) then
				write(*,'(A,I3)')  ' ITER # :',i
				if(i==1)     write(*,*) '   - Elapsed time for FISHPACK :', toc-tic, ' s'   
				if(i==1)     write(*,*) '   - PERTRB, ot%ierr :', sngl(PERTRB), ierr   
			...
			fcost_prev=fcost
			fcost=-sum(x*L)
			if (convergence>0. .and. i>10) then
			if (abs( (fcost-fcost_prev)/fcost_prev ) <= convergence) then
				! call hud(shot%sindex//)
		...
		if (i==niterate+1) write(*,'(A,I4,A,E10.5,A,E10.5)')     ' Proc #',mype, ' MAX ITERation reached.  Inner misfit :', fcost, '; Norm of F :',maxval(abs(F))
