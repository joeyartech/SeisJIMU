...
use m_field
    ...
    if(mpiworld%is_master) then
        ...
        write(*,'(a)') "----------------------------"
        write(*,'(a)') ""
        write(*,'(a)') "bash $< export MKL_NUM_THREADS=1 #(use source in csh)"
        ...
        write(*,'(a)') "NSHOTS                  1    #Number of shots to be inverted. Shot "
        ...
        write(*,'(a)') "SMOOTHING_ADDMIRROR     dz   #Increase this value to avoid leakage into the mask zone after smoothing"
        ...
        write(*,'(a)') "IF_REINITIALIZE_ALPHA   T/F                 #If reset alpha=1 at each new line search procedure (=T for l-BFGS, =F for NLCG)"
